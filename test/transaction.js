...
describe('getTransactionProof', () => {
  ...
  it('should be able to request a proof for 0xb53f7522 and verify it', async () => {
    ...
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });
  ...
  it('should be able to request a proof for 0xdaa2fcc5 and verify it', async () => {
    let prf = await buildProof.getTransactionProof('0xfd7f67e10bb48c641743108096eb7aa750d17afb8ac95560d93ebcd347e74443')
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });

  it('should be able to request a proof for 0xdaa2fcc5 and verify it', async () => {
    let prf = await buildProof.getTransactionProof('0xc1080d2eaf1f3e866a4c12298a0be47647665a120d8ee681520eb440a6e06f99')
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });

  it('should be able to request a proof for 0xdaa2fcc5 and verify it', async () => {
    let prf = await buildProof.getTransactionProof('0xe6a37c02c198f9e4c8e7831a1b6a0e6711bf372a301147da339e2f61117f58a1')
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });

  it('should be able to request a proof for 0xdaa2fcc5 and verify it', async () => {
    let prf = await buildProof.getTransactionProof('0x7392ee6d4bea6a0c8018de116fd2d6bf5678fdc0faae53240e7a22ab57db22d0')
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });

  it('should be able to request a proof for 0xdaa2fcc5 and verify it', async () => {
    let prf = await buildProof.getTransactionProof('0xacb81623523bbabccb1638a907686bc2f3229c70e3ab51777bef0a635f3ac03f')
    VerifyProof.transaction(prf.path, prf.value, prf.branch, prf.header, prf.blockHash).should.be.true()
  });
  ...
  it('verifying a modified or different proof should throw an error', async () => {
    let prf = await buildProof.getTransactionProof('0x4e4b9cd37d9b5bb38941983a34d1539e4930572bdaf41d1aa54ddc738630b1bb')
    let otherPrf = await buildProof.getTransactionProof('0x74bdf5450025b8806d55cfbb9b393dce630232f5bf87832ae6b675db9d286ac3')

    expect(()=>{VerifyProof.transaction(Buffer.from('80','hex'), prf.value, prf.branch, prf.header, prf.blockHash)}).to.throw()
    expect(()=>{VerifyProof.transaction(prf.path, otherPrf.value, prf.branch, prf.header, prf.blockHash)}).to.throw()
