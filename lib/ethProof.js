    ...
    self.web3.eth.getBlock(receipt.blockHash, false, function(e,block){
      ...
      async.map(block.transactions,function(siblingTxHash, cb2){
        ...
        receiptsTrie.get(rlp.encode(receipt.transactionIndex), function(e,rawReceipt,stack){
          thing = rawStack(stack)[stack.length-1]

          console.log(sha3(rlp.encode(stack[0].raw)) == block.receiptsRoot.slice(2), sha3(rlp.encode(stack[0].raw)))
          // console.log("TRIE_ROOT_FROM_TRIE", receiptsTrie.root)
          // console.log("TRIE_ROOT_FROM_STACK", sha3(rlp.encode(stack[0].raw)))
          var prf = {
            blockhash: new Buffer(receipt.blockHash.slice(2),'hex'),
            header:    getRawHeader(block),
            stack:     rawStack(stack),
            path:      rlp.encode(receipt.transactionIndex),
            receipt:   rlp.decode(rawReceipt)
          }
          cb(e, prf)
        })
      });
...
var putReceipt = (siblingReceipt, receiptsTrie, cb2) => {//need siblings to rebuild trie
  ...
  var postTransactionState = Buffer.from(siblingReceipt.root.slice(2),'hex')
  var cummulativeGas = Buffer.from(siblingReceipt.cumulativeGasUsed.toString(16),'hex')
  var setOfLogs = encodeLogs(siblingReceipt.logs)
  var bloomFilter = Buffer.from(siblingReceipt.logsBloom.slice(2),'hex')
  // console.log("RECEIPTSTHING",[postTransactionState,cummulativeGas,setOfLogs,bloomFilter])
...
var encodeLogs = (input) => {
  ...
  for (var i = 0; i < input.length; i++) {
    // throw new Error("HASLOG")
    var address = Buffer.from(input[i].address.slice(2), 'hex');
    var topics = input[i].topics.map((topic)=>{
      return Buffer.from(topic.slice(2),'hex')
    })
    var data = Buffer.from(input[i].data.slice(2),'hex')
    logs.push([address, topics, data])
  }
  // [[address, topics[], data], [address, topics[], data]]
  console.log("LOGS: ", logs)
    // if(logs[0]){ throw new Error("HASLOG") }
  return logs
  // return []
}
...
var getRawHeader = (_block) => {
  // console.log("BLOCKHEADER", _block)
