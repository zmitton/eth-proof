...
EV.accountNonce = (address, nonce, account, stack, header, blockHash) => {
  return EV._valueInAccountIndex(0, address, nonce, account, stack, header, blockHash)
}
EV.balance = (address, balance, account, stack, header, blockHash) => {
  return EV._valueInAccountIndex(1, address, balance, account, stack, header, blockHash)
}
EV.storageRoot = (address, storageRoot, account, stack, header, blockHash) => {
  return EV._valueInAccountIndex(2, address, storageRoot, account, stack, header, blockHash)
}
EV.codeHash = (address, codeHash, account, stack, header, blockHash) => {
  return EV._valueInAccountIndex(3, address, codeHash, account, stack, header, blockHash)
}
EV.byteCode = (address, byteCode, account, stack, header, blockHash) => {
  try{
    if(Buffer.from(sha3(rlp.encode(byteCode)),'hex').equals(valueFrom(stack))){
      return EV.codeHash(address, byteCode, stack, header, blockHash)
    }
  }catch(e){ console.log(e) }
  return false
}
EV._valueInAccountIndex = (accountIndex, address, value, account, stack, header, blockHash) => {
  try{
    var account = EV.valueFrom(stack) // decoded last last
    if(EV.account(address, account, stack, header, blockHash)){
      return account[accountIndex].equals(value)
    }
  }catch(e){ console.log(e) }
  return false
}
...
EV._valueInTrieIndex = (trieIndex, path, value, stack, header, blockHash) => {
  try{
    ...
    if(EV.headerValue(trieIndex, trieRoot, header, blockHash)){
      // console.log("THERE",path, value, stack, trieRoot)
