...
EV.accountNonce = (address, nonce, stack, header, blockHash) => {
  return EV._accountElement(0, address, nonce, stack, header, blockHash)
}
EV.balance = (address, balance, stack, header, blockHash) => {
  return EV._accountElement(1, address, balance, stack, header, blockHash)
}
EV.storageRoot = (address, storageRoot, stack, header, blockHash) => {
  return EV._accountElement(2, address, storageRoot, stack, header, blockHash)
}
EV.codeHash = (address, codeHash, stack, header, blockHash) => {
  return EV._accountElement(3, address, codeHash, stack, header, blockHash)
}
EV.code = (address, code, stack, header, blockHash) => {
  try{
    var account = EV.valueFrom(stack)
    if(Buffer.from(sha3(code),'hex').equals(account[3])){
      return EV.codeHash(address, account[3], stack, header, blockHash)
    }
  }catch(e){ console.log(e) }
  return false
}

EV._accountElement = (accountIndex, address, targetValue, stack, header, blockHash) => {
  try{
